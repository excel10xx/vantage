...
const { withdrawFromWallet, exchangeCurrency, transfer, depositIntoWallet } = require('../controllers/actions/walletActionsController');
...
router.post('/wallet/withdraw', authenticate, async (req, res) => {
    ...
    try {
        const result = await withdrawFromWallet(userId, currency, chain, amountInUSD, method);
...
router.post('/wallet/exchange', authenticate, async (req, res) => {
    ...
    const userId = req.user._id;
    try {
        const result = await exchangeCurrency(userId, fromCurrency, fromChain, toCurrency, toChain, amount);
        res.status(result.code).json(result);
    } catch (error) {
        res.status(error.code || 500).json({ success: false, message: error.message });
    }
...
router.post('/wallet/deposit', authenticate, async (req, res) => {
    ...
    const userId = req.user._id;
    try {
        const result = await depositIntoWallet(userId, amountInUSD, currency, chain);
        res.status(result.code).json(result);
    } catch (error) {
        res.status(error.code || 500).json({ success: false, message: error.message });
    }

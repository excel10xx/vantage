...
const createExpertTrader = async (req, res) => {
    try {
        const { name, profilePicture, totalProfit, todaysProfit, profitShare, followersEquity, minimumCapital, rating, winRate } = req.body;
        ...
        const newExpertTrader = await ExpertTrader.create({
            name,
            profilePicture: profilePicture || 'public/profile-pictures/defaultpicture.jpg',
...
const getAllExpertTraders = async (req, res) => {
    try {
        const expertTraders = await ExpertTrader.find().populate('followers', 'name email');
...
const getExpertTraderById = async (req, res) => {
    try {
        const { id } = req.params;
        const expertTrader = await ExpertTrader.findById(id).populate('followers', 'name email');
...
const updateExpertTrader = async (req, res) => {
    try {
        ...
        const updatedExpertTrader = await ExpertTrader.findByIdAndUpdate(id, updates, { new: true, runValidators: true });
